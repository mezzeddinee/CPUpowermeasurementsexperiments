...
def stress_memory(target_fraction):
    ...
    Uses bytearrays for realistic physical memory allocation.
    """
    ...
    try:
        ...
        # Touch each block to ensure it's physically committed
        for b in allocated:
            b[0] = 0
        return allocated
    except MemoryError:
...
def main():
    parser = argparse.ArgumentParser(
        description="Stress CPU and/or Memory and measure RAPL core-domain energy."
    )
    parser.add_argument("a", type=float, help="CPU usage fraction (0–1)")
    parser.add_argument("--mem", type=float, default=0.0,
                        help="Memory usage fraction (0–1, default=0)")
    ...
    if not (0 <= a <= 1 and 0 <= m <= 1):
        print("Error: Fractions must be between 0 and 1.")
        return
    ...
    print(f"\n--- STARTING STRESS: CPU={a*100:.0f}%, MEM={m*100:.0f}%, DURATION={duration}s ---\n")
    processes = stress_cpu(a) if a > 0 else []
    mem_alloc = stress_memory(m) if m > 0 else []
